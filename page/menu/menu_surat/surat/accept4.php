<?php
	...
	if (isset($_POST['bukti'])){
		...
		$sql = "UPDATE surat SET no_reg='$no_reg',tgl='$tgl',nama_file4='$newname' WHERE id = '$appid'";
		$run = mysqli_query($koneksi1,$sql);
		if($run == true){
			
			$tgl=date("Y-m-d H:i:s");
			mysqli_query($koneksi1,"UPDATE surat_status 
						SET status_sekertarisakhir='Bukti Bayar Sanksi Selesai',
							id_usersekertarisakhir='$_SESSION[iduser]',
							tgl_updatesekertarisakhir='$tgl' 
						WHERE id_surat='$appid'");
						
			
			$_SESSION['sukses']="File tanda terima berhasil diupload";
